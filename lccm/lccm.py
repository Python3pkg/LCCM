...
def sampleEnum(sampleEnumFile, nClasses, nInds, inds, paramClassMem, expVarsClassMem, indClassAv,
    paramClassSpec, expVarsClassSpec, altAv, obsAv, rowAv, nAlts):
    """
    Function that performs sample enumeration across all individuals in the sample for both
    the class specific choice model and class membership model.
    
    Parameters
    ----------
    sampleEnumFile : string.
        File path to where all the output files should be stored and name of 
        csv file of sample enumeration results.
    nClasses : Integer.
        Number of classes to be estimated by the model.
    nInds : Integer.
        Total number of individuals/decision-makers in the dataset.
    ...
    Returns
    -------
    p : 2D numpy array of size nInds x [(nAlts x nClasses) + (nClasses) + 1].
        The (i, j)th element of the returned 2D array denotes:
            1- individual ID in the sample, if j=1
            2- probability that individual i belongs to the corresponding ordered latent
               class, if 2 <= j <= (1 + nClasses)
            3- probability that individual i chooses the corresponding ordered alternative,
               if (1 + nClasses) < j <= (nAlts x nClasses) + (1 + nClasses)
            

    """      

    pIndMod = calClassMemProb(paramClassMem, expVarsClassMem, indClassAv).reshape((nInds, nClasses), order = 'C')

    p = enumClassSpecificProbPanel(paramClassSpec[0], expVarsClassSpec[0], altAv[0], obsAv, rowAv, nInds, nAlts)
    for s in range(1, nClasses):
        p = np.hstack((p, enumClassSpecificProbPanel(paramClassSpec[s], expVarsClassSpec[s],
        altAv[s], obsAv, rowAv, nInds, nAlts)))
    p = np.hstack((inds[:, None], pIndMod, p))
    np.savetxt(sampleEnumFile, p, delimiter = ',') 
...
def emAlgo(outputFilePath, outputFileName, outputFile, nClasses, 
    ...
    # defining an original expVarsClassMem to be used later for sample enumeration
    expVarsClassMem_original_shape = expVarsClassMem
    ...
    while not converged:
        ...
        paramClassMem = minimize(wtClassMem, paramClassMem, args = (expVarsClassMem, indClassAv, 
                weights.reshape((nClasses * nInds, 1), order = 'F'), cWeights.reshape((nClasses * nInds,1),
                                                                                      order = 'F')), 
    ...
    (nInds, expVarsClassMem, indClassAv,
    altAv, altChosen, obsAv, rowAv)\
    = processData(inds, indID, nClasses, expVarsClassMem_original_shape, availIndClasses,
    obsID, altID, choice, availAlts)
    
    nAlts = np.unique(altID).shape[0]

    # performing sample enumeration to output the required csv file (ModelResultsSampleEnum.csv)
    sampleEnum(outputFilePath + outputFileName + 'SampleEnum.csv',
    nClasses, nInds, inds,
    paramClassMem, expVarsClassMem, indClassAv,
    paramClassSpec, expVarsClassSpec, altAv, obsAv, rowAv, nAlts)
